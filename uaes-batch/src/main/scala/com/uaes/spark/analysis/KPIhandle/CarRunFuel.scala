...
/**
  ...
  */
object CarRunFuel {
  val logger = LoggerFactory.getLogger(CarRunFuel.getClass)
  ...
  def main(args: Array[String]): Unit = {
   ...
   val (startTime ,endTime) = TimeUtils.currentTime(args)
    ...
    val sc = SparkUtil.getSparkContext("CarRunFuel", logger)
    ...
    everyDayCarRunFuel(filterEverydayRdd)
    everyHunKiCarRunFuel(filterEveryHunKilRdd)
  }

  //每天正常行驶耗油
  def everyDayCarRunFuel(rDD: RDD[String]): Unit = {
  ...
  //每百公里正常行驶耗油
  def everyHunKiCarRunFuel(rDD: RDD[String]): Unit = {
    ...
    val resRDD = resDF.rdd.map(row => {
      val vin = row.getString(0)
      val everyHunKil = row.getLong(1).toString
      val KPI = "每百公里正常耗油"
      val totalFuel = row.getDouble(2)
